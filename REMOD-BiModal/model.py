...
class JointModel(nn.Module):
    def __init__(self, config, g, h, 
            score_func, label_num, hidden_dim, dropout):
        super().__init__()
        self.g = g
        self.h = h
        self.config = config
        ...
        self.graph_scorer = Scorer(score_func, label_num, hidden_dim, dropout)
    ...
    def forward(self, cui1, cui2, tokens, split_points, entity_1_begin_idxs, entity_2_begin_idxs):
        ...
        score_text = self.text_scorer(h_text, t_text)
        score_graph = self.graph_scorer(h_graph, t_graph)
        return score_text, score_graph


    def __repr__(self):
        return "_".join(("JOINT", self.config, self.score_func, str(self.hidden_dim), str(self.dropout)))
